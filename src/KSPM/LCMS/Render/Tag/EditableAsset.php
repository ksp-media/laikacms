...
class EditableAsset extends Tag implements TagInterface{
    
    protected $_pattern = '/@editable_asset\(\'(.*?)\'\)(.*?)@endeditable_asset/is';
    ...
    public function handleResult(){
        foreach ($this->_results as $r) {
            ...
            $this->addTmplObj(array('origin' => $r[0], 'key' => $r[1], 'value' => $c, 'type' => "input_file"));
        }
    }
