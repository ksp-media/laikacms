    ...
    public function loginAction() {
        ...
        if ($_POST['user']['email'] && $_POST['user']['password']) {
            ...
            if ($user) {
               ...
               return \Redirect::away("/"._LCMS_PREFIX_."/");
            } else {
              return  \Redirect::to('/'._LCMS_PREFIX_.'/login')->with('message', 'Login Failed: user not found');
            }
        } else {
           return \Redirect::away('/'._LCMS_PREFIX_.'/login')->with('message', 'Login Failed');
        }
    }
    
    public function logoutAction() {
        \Session::forget('cmslogin');
        return \Redirect::away('/'._LCMS_PREFIX_.'/login');
    }
