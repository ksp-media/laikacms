...
$('document').ready(function () {
    ...
    $('#btn-save').click(function () {
        $.post('/' + jbkcms_prefix + '/cms/content/create', $('#content-form').serialize(), function (response) {
    ...
    $('.clear-cache').click(function () {
        $.get('/' + jbkcms_prefix + '/cms/clearcache', function () {
    ...
    if ($('#page-structure')) {
        ...
        $('.tree-item').each(function () {
            ...
            $(this).on('click', function () {
                location.href = '/' + jbkcms_prefix + '/cms/page/' + $(this).attr('data-id') + '/edit';
            });
        });
        $('#page-structure').on('change', function () {
            $.post('/' + jbkcms_prefix + '/cms/page/updatetree', {'pagetree': $('#page-structure').nestable('serialize')}, function (response) {
            })
        });
    }
...
Jbkcms.User = {
    loadUserDetailModal: function (userid) {
        $.get('/' + jbkcms_prefix + '/user/form/' + userid, function (response) {
            $('#user-form-modal').remove();
            $('body').append(response);
            $('#user-form-modal').modal('show')
            $('#user-form-modal-save').unbind('click');
            $('#user-form-modal-save').click(function () {
                $.post('/' + jbkcms_prefix + '/user/form', $('#user-form').serialize(), function (response) {
